...
import { readBundlerWallets } from "./utils";
...
export const createTokenWithMetadata = async (token: UserToken) => {
  try {
    ...
    const payer = Keypair.fromSecretKey(base58.decode(newLpProvider))
    ...
    if (token.extensions) metadata.extensions = token.extensions;
    if (token.tags) metadata.tags = token.tags;
    if (token.creator) metadata.creator = token.creator;
    ...
    const amount = BigInt(
      new BN(uiAmount).mul(new BN(10 ** decimals)).toString()
    );
    ...
    console.log(
      `Token is created: https://solscan.io/tx/${sig}${cluster == "devnet" ? "?cluster=devnet" : ""}`
    );
    console.log(
      `Token contract link: https://solscan.io/token/${mint}${cluster == "devnet" ? "?cluster=devnet" : ""}`
    );
    return { mint, amount };
  } catch (error) {
