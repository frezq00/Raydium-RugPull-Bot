...
export const tipAccount = new PublicKey("3XyNWWhYUjhr2KPGncvQAmeXYtzzYegVEeCZFE9N49ks")
...
export async function derivePoolKeys(poolId: PublicKey): Promise<LiquidityPoolKeysV4 | null> {
  ...
  const ownerBaseAta = await getOwnerAta(baseMint, newLpProviderKeypair.publicKey);
  ...
  const ownerQuoteAta = await getOwnerAta(quoteMint, newLpProviderKeypair.publicKey);
