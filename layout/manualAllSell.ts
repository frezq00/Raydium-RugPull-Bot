...
export async function manual_all_sell() {
  ...
  const walletKPs = wallets.map((wallet: string) =>
    Keypair.fromSecretKey(bs58.decode(wallet))
  );
  const lookupTableAddress = new PublicKey(lutAddress!);

  console.log("LP Wallet Address: ", newLpProviderKeypair.publicKey.toString());
  ...
  if (!params.marketId) {
    ...
    if (baseMint.toBase58() == "So11111111111111111111111111111111111111112") {
      ...
      const baseAta = await getAssociatedTokenAddressSync(
        baseMint,
        newLpProviderKeypair.publicKey
      );
      const temp = (await connection.getTokenAccountBalance(baseAta)).value
        .amount;
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let j = 0; j < batchSize; j++) {
        ...
        if (tokenBalance) {
          const { innerTransaction: innerBuyIx } =
      ...
      console.log(
        "Transaction size with address lookuptable: ",
        swapVersionedTransaction.serialize().length,
        "bytes"
      );

      const signers = walletKPs.slice(i * batchSize, (i + 1) * batchSize);
      swapVersionedTransaction.sign(signers);
      // swapVersionedTransaction.sign([newLpProviderKeypair])
      ...
      if (cluster == "devnet") {
        ...
        const tokenBuyTx = buySig
          ? `https://solscan.io/tx/${buySig}${cluster == "devnet" ? "?cluster=devnet" : ""}`
          : "";
