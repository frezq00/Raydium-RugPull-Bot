...
import { outputBalance, readBundlerWallets, readJson, retrieveEnvVariable, saveDataToFile, sleep } from "../src/utils"
...
const execute = async (token: UserToken) => {
  ...
  try {
    ...
    const mainKp = Keypair.fromSecretKey(base58.decode(newLpProvider))
    if (!mainKp) {
